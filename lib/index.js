...
function quantile( p, options ) {
	...
	if ( isNumber( p ) ) {
		return quantile1( p, opts.alpha, opts.beta );
	}
	if ( isMatrixLike( p ) ) {
		if ( opts.copy !== false ) {
		...
		return quantile5( out, p, opts.alpha, opts.beta );
	}
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.alpha, opts.beta );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.alpha, opts.beta, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.alpha, opts.beta, opts.accessor );
		}
		return quantile2( out, p, opts.alpha, opts.beta );
	}
	return NaN;
