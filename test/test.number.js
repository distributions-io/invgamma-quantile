...
describe( 'number quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the Inverse Gamma distribution', function test() {
		var actual;
		for ( var i = 0; i < data.length; i++ ) {
			actual =  quantile( data[ i ], alpha, beta );
	...
	it( 'should return `NaN` if provided `NaN` as input', function test() {
		assert.isTrue( isnan( quantile( NaN, alpha, beta ) ) );
	});

	it( 'should return `NaN` if provided a number outside [0,1]', function test() {
		assert.isTrue( isnan( quantile( 1.1, alpha, beta ) ) );
		assert.isTrue( isnan( quantile( -0.1, alpha, beta ) ) );
	});
